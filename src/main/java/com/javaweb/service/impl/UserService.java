...
public class UserService implements IUserService {
    ...
    public UserDTO findOneByUserNameAndStatus(String name, int status) {
        UserEntity entity = userRepository.findOneByUserNameAndStatus(name, 1);
        if (entity == null) {
            return null; // phải kiểm tra null
        }
    ...
    public Map<Long, String>  getStaffs() {
        ...
        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(SystemConstant.STATUS, SystemConstant.STAFF);
    ...
    public UserDTO findOneByUserName(String userName) {
        ...
        UserDTO result = userConverter.convertToDto(userEntity);
        return result;
    }
    ...
    public void delete(long[] ids) {
        for (Long item : ids) {
            UserEntity userEntity = userRepository.findById(item)
                    .orElseThrow(() -> new ResourceNotFoundException("User not found with id " + item));
